...
const steps = [
  { key: "face",      label: "Face Identity",    short: "Face" },
  { key: "ic",        label: "IC Card Reference", short: "IC" },
  { key: "workspace", label: "Workspace Layout",  short: "Work" },
  { key: "hands",     label: "Hand Biometrics",   short: "Hands" },
  { key: "complete",  label: "Verified",          short: "Done" },
];

const stepCopy: Record<string, { title: string; body: string }> = {
  face: {
    title: "Identity Verification",
    body: "Position your face within the frame. Our zero-trust engine will capture a forensic biological reference to lock this session to your identity.",
  },
  ic: {
    title: "Document Reference",
    body: "Hold your IC or National ID card flat within the guide. Ensure all text and the hologram are visible for tamper-evidence verification.",
  },
  workspace: {
    title: "Environment Audit",
    body: "Capture a clear view of your workspace, including tools and components. This establishes the forensic baseline for the hands-on session.",
  },
  hands: {
    title: "Biometric Hand Lock",
    body: "Place both hands in the frame. The system will map unique biological markers (nail beds, skin texture, and geometry) to ensure continuous presence.",
  },
};
...
export default function IdentityVerificationPage() {
  ...
  const step = (searchParams.get("step") || "face") as string;
  const setStep = useCallback((s: string) => {
  ...
  useEffect(() => {
    ...
    setLogs([
      { time, msg: "Verification pipeline active." },
            ...
            {step === "ic" && (
              <div className="absolute inset-0">
                <svg className="absolute inset-0 h-full w-full" viewBox="0 0 16 9" preserveAspectRatio="xMidYMid slice">
                  ...
                  <rect x="4.2" y="2.0" width="7.6" height="5.0" rx="0.3" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="0.04" />
                ...
                <div className="absolute bottom-5 inset-x-0 flex justify-center pointer-events-none">
                  <div className="rounded-full bg-black/65 px-5 py-2 backdrop-blur-sm">
                    <p className="text-[11px] font-medium tracking-wide text-white">Hold the IC card flat | avoid glare</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {step === "face" && (
              <div className="absolute inset-0">
                <svg className="absolute inset-0 h-full w-full" viewBox="0 0 16 9" preserveAspectRatio="xMidYMid slice">
                  ...
                  {faceState === "countdown" && (
                    <text x="8" y="5.15" textAnchor="middle" fill="white" fontSize="2.2" fontWeight="600" fontFamily="system-ui">
                      {faceCountdown}
                    </text>
                  )}
                  {faceState === "scanning" && (
                  ...
                  {(faceState === "scanning" || faceState === "captured") && (
                    <path d="M 8 1.65 A 2.15 2.85 0 0 1 10.15 4.5 A 2.15 2.85 0 0 1 8 7.35 A 2.15 2.85 0 0 1 5.85 4.5 A 2.15 2.85 0 0 1 8 1.65"
                      fill="none" stroke={faceState === "captured" ? "#22c55e" : "white"} strokeWidth="0.07" strokeLinecap="round"
                      strokeDasharray="16 100" strokeDashoffset={faceState === "captured" ? "0" : "16"}
                  ...
                  {faceState === "captured" && (
                    <g>
                      ...
                      <path d="M7.68,4.52 L7.9,4.74 L8.38,4.2" stroke="white" strokeWidth="0.1" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                    </g>
                  )}
                </svg>
                {faceState === "captured" && ( <div className="absolute inset-0 bg-white/15 pointer-events-none" /> )}
                <div className="absolute bottom-5 inset-x-0 flex justify-center pointer-events-none">
                  <div className="rounded-full bg-black/65 px-5 py-2 backdrop-blur-sm">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="relative h-[72%] w-[84%]">
                  <svg className="absolute inset-0 w-full h-full overflow-visible" viewBox="0 0 100 60" preserveAspectRatio="none">
                    ...
                    <g opacity="0.12" fill="white"> <ellipse cx="26" cy="30" rx="14" ry="18" /> </g>
                    <g opacity="0.12" fill="white"> <ellipse cx="74" cy="30" rx="14" ry="18" /> </g>
                  </svg>
